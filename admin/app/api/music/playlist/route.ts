...
export async function GET(request: NextRequest) {
  ...
  const playlistId = trimString(searchParams.get('id'));
  ...
  try {
    const url = `${SONG_API_BASE_URL}/playlists?id=${playlistId}`;
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch playlist details from JioSaavn: ${response.statusText}`);
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        artist: decodeHtmlEntities(data.firstname || data.lastname || data.username || 'JioSaavn'),
        imageUrl: Array.isArray(data.image) ? getMediaUrl(data.image[data.image.length - 1]) : data.image || '',
        songs: normalizedSongs,
      },
    });
  } catch (error: any) {
    console.error('[playlist-songs-fetch]', error);
    return NextResponse.json(
      { success: false, message: error.message || 'Failed to fetch playlist songs.' },
      { status: 500 }
    );
  }
}
